...
for j=1:N
    ...
    b = 0.5 + rand / 2;
    %a = 0.9 * (2 * rand - 1); % a between -0.9 and 0.9
    
    %u0 = chebfun(@(x) 1/2 * (a * ubase(x - b) + sqrt(1 - a^2) * ubase2(x - b)), [0 1]);
    u0 = chebfun(@(x) 1/2 * (a * ubase(x/b) + sqrt(1 - a^2) * ubase2(x/b)), [0 1]);
    
    u = heat1(u0, tspan, s, heat, 1);
...
grfarc2heat = output(:, 1:1:21, :);
save("grfarc2heat.mat", "grfarc2heat", "ubase", "ubase2", "params", "a1", "a2", "b1", "b2");

function u = heat1(init, tspan, s, visc, plot)
